...
export function* fetchFasit(action) {
  const resourcesConfig = yield select(
  ...
  try {
    ...
    if (action.revision) {
      value = yield call(
        fetchUrl,
        `${resourcesConfig}/${action.id}/revisions/${action.revision}`
      )
    } else {
...
export function* fetchFasitResourceSecret() {
  try {
    ...
    if (
      !isEmptyObject(resource) &&
      validAuthorization(user, resource.data.accesscontrol)
    ) {
      const secretRefs = yield select(
        (state) => state.resource_fasit.data.secrets
      )
