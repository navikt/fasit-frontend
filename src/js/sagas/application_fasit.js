...
export function* fetchFasit(action) {
  const applications_api = yield select(
    (state) => state.configuration.fasit_applications
  );
  let value = {};
  ...
  try {
    if (action.revision) {
      value = yield call(
        fetchUrl,
        `${applications_api}/${action.name}/revisions/${action.revision}`
      );
    } else {
...
export function* fetchApplicationInstances(action) {
  const instances_api = yield select(
  ...
  try {
    const value = yield call(
      fetchUrl,
      `${instances_api}/application/${action.name}`
    );
