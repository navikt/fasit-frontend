...
export function* fetchEnvironment(action) {
  ...
  try {
    if (action.revision) {
      value = yield call(
        fetchUrl,
        `${environmentsApi}/${action.id}/revisions/${action.revision}`
      );
    } else {
...
export function* fetchEnvironmentClusters(action) {
  ...
  try {
    const value = yield call(
      fetchUrl,
      `${environmentsApi}/${environment}/clusters`
    );
...
export function* fetchEnvironmentCluster(action) {
  ...
  try {
    if (action.revision) {
      value = yield call(
        fetchUrl,
        `${environmentsApi}/${environment}/clusters/${cluster}/revisions/${revision}`
      );
    } else {
      value = yield call(
        fetchUrl,
        `${environmentsApi}/${environment}/clusters/${cluster}`
      );
    }
    yield put({ type: ENVIRONMENT_CLUSTER_FASIT_RECEIVED, value });
  } catch (error) {
...
export function* fetchEnvironmentNodes(action) {
  ...
  try {
    const value = yield call(
      fetchUrl,
      `${nodesApi}?environment=${action.environment}&pr_page=1000`
    );
...
export function* fetchEnvironmentInstances(action) {
  ...
  try {
    const value = yield call(
      fetchUrl,
      `${instancesApi}/environment/${action.environment}`
    );
