...
describe('(Component) Environment', () => {
    ...
    it('(function) "rescue" dispatches correct action', () => {
        ...
        expect(dispatch.args[0][0].key).to.equal(103373)
        expect(dispatch.args[0][0].elementType).to.equal("environment")
    })
    ...
    it('(function) "handleSubmitForm" dispatches correct action', () => {
        ...
        wrapper.setState({displayAccessControlForm: true})
        expect(wrapper.state().displayAccessControlForm).equal(true)
    ...
    it('(function) "componentWillReceiveProps" dispatches correct action', () => {
        const dispatch = sinon.spy()
        const wrapper = shallow(<Environment {...props} dispatch={dispatch}/>)
        wrapper.instance().componentWillReceiveProps({name: "mikke mus", environment: [], query: {revision: ""}})
        expect(dispatch.args[0][0].type).to.equal("ENVIRONMENT_FASIT_REQUEST")
        expect(dispatch.args[0][0].id).to.equal("p")
    })

    it('renders "CurrentRevision" with props', () => {
        const wrapper = shallow(<Environment {...props} query={{revision: "mikke"}} revisions="mus"/>)
    ...
    it('renders "ToolButtons" with props', () => {
        ...
        expect(wrapper.find(ToolButtons).props().onEditClick).to.be.instanceof(Function)
        expect(wrapper.find(ToolButtons).props().onDeleteClick).to.be.instanceof(Function)
        expect(wrapper.find(ToolButtons).props().onCopyClick).to.be.instanceof(Function)
...
const props = {
    ...
    "environment": {
        ...
        "links": {
            ...
            "revisions": "https://e34jbsl01655.devillo.no:8443/api/v2/environments/p/revisions",
            "clusters": "https://e34jbsl01655.devillo.no:8443/api/v2/environments/p/clusters"
        }
    },
    "environmentClasses": [
        "u",
        ...
        "data": [
            {
                ...
                "revisiontype": "mod",
                "links": {
                    "entity": "https://e34jbsl01655.devillo.no:8443/api/v2/environments/p/revisions/3236864"
                }
            },
            {
                "revision": 3236863,
                ...
                "revisiontype": "mod",
                "links": {
                    "entity": "https://e34jbsl01655.devillo.no:8443/api/v2/environments/p/revisions/3236863"
                }
            },
            {
                "revision": 3236860,
                ...
                "revisiontype": "mod",
                "links": {
                    "entity": "https://e34jbsl01655.devillo.no:8443/api/v2/environments/p/revisions/3236860"
                }
            },
