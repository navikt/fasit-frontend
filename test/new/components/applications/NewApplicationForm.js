...
describe('(Component) NewApplicationForm', () => {
    ...
    it('(function) "handleSubmitForm" dispatches action based on props.mode copy', () => {
        ...
        const wrapper = shallow(<NewApplicationForm {...props} dispatch={dispatch}/>)
        wrapper.setState({
            name: "1",
            artifactid: "2",
            groupid: "3",
    ...
    it('(function) "handleSubmitForm" dispatches action based on props.mode edit', () => {
        const dispatch = sinon.spy(() => {
        })
        const wrapper = shallow(<NewApplicationForm {...props} dispatch={dispatch} mode={"edit"}/>)
        wrapper.instance().handleSubmitForm()
        expect(dispatch.callCount).to.equal(1)
        expect(dispatch.args[0][0].component).to.equal('application')
    })

    it('(function) "closeForm" dispatches action', () => {
        const dispatch = sinon.spy(() => {
        })
        const wrapper = shallow(<NewApplicationForm {...props} dispatch={dispatch}/>)
